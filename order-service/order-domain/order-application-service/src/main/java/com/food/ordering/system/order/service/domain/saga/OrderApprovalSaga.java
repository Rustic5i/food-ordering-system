...
public class OrderApprovalSaga implements SagaStep<RestaurantApprovalResponse> {
    ...
    public void process(RestaurantApprovalResponse restaurantApprovalResponse) {
        ...
        if (orderApprovalOutboxMessageResponse.isEmpty()) {
            log.info("Исходящее сообщение с идентификатором saga: {} уже обработано!",
        ...
        log.info("Заказ с идентификатором: {} одобрен.", order.getId().getValue());
    }
    ...
    public void rollback(RestaurantApprovalResponse restaurantApprovalResponse) {
        ...
        if (orderApprovalOutboxMessageResponse.isEmpty()) {
            log.info("Исходящее сообщение с идентификатором саги: {} уже отменено!",
                    restaurantApprovalResponse.getSagaId());
            return;
        }
        ...
        log.info("Заказ с идентификатором: {} отменяется", domainEvent.getOrder().getId().getValue());
    }

    private Order approvalOrder(RestaurantApprovalResponse restaurantApprovalResponse) {
        log.info("Заказ на утверждение с идентификатором: {}", restaurantApprovalResponse.getOrderId());
    ...
    private OrderPaymentOutboxMessage getUpdatedPaymentOutboxMessage(String sagaId,
        ...
        if (orderPaymentOutboxMessageResponse.isEmpty()) {
            String errorMessage = "Исходящее сообщение о платеже со статусом {} не найдено".formatted(SagaStatus.PROCESSING.name());
    ...
    private OrderCancelledEvent rollbackOrder(RestaurantApprovalResponse restaurantApprovalResponse) {
        log.info("Отмена заказа с идентификатором: {}", restaurantApprovalResponse.getOrderId());
        Order order = orderSagaHelper.findOrder(restaurantApprovalResponse.getOrderId());
